...
hide, project, pk                          & schema calculus / annotation \\
...
{\scriptsize \kw{Bank} has \texttt{counters : ID -> Counter}; promotion lifts a local \kw{Bump} into one entry:}
...
1  & postfix \texttt{\textasciitilde}, \texttt{+}, \texttt{*}, \texttt{\^{}} (inverse, closures, iter) \\
2  & prefix \texttt{dom}, \texttt{ran}, \texttt{\#}, \texttt{P}, \texttt{F}, \texttt{seq}, \texttt{bigcup}, \texttt{bigcap}, \texttt{inv}, \texttt{id} \\
3  & postfix at atom level: function application \texttt{f(x)}, relational rename \texttt{R[B/A]}, relational atoms \texttt{sigma[..]}, \texttt{pi[..]} \\
4  & \texttt{*}, \texttt{mod} \\
5  & \texttt{+}, \texttt{-} (binary) \\
6  & range \texttt{..} \\
7  & \texttt{cross}, \texttt{join}, \texttt{div} (cross-product / relational) \\
8  & \texttt{union}, \texttt{setminus} (\texttt{\textbackslash}) \\
9  & \texttt{intersect} \\
10 & relational \texttt{<->}, \texttt{|->}, \texttt{<|}, \texttt{|>}, \texttt{<<|}, \texttt{|>>}, \texttt{o9}, \texttt{comp} \\
11 & function arrows \texttt{->}, \texttt{+->}, \texttt{>->}, \texttt{>->>}, \texttt{-->>}, \texttt{+->>}, \texttt{77->} (finite) \\
