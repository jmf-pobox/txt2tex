...
$Expr$ & $::=$ & $const \llangle \nat \rrangle \mid binop \llangle OP \cross Expr \rrangle$
\end{tabular}
\end{exget}

{\scriptsize \kw{syntax} produces a column-aligned fuzz \texttt{syntax} environment. Blank lines between definitions emit \texttt{\textbackslash also} (visual group separator).}
...
\smallskip
{\scriptsize\textbf{Fuzz note:} binding expressions emit outside any Z environment — fuzz silently skips them. Do not wrap bindings inside \texttt{zed}/\texttt{axdef}/\texttt{schema} blocks.}
...
{\scriptsize\textbf{Fuzz note:} algebra expressions emit outside any Z environment — fuzz silently skips them. Do not wrap algebra inside \texttt{zed}/\texttt{axdef}/\texttt{schema} blocks.}
...
{\scriptsize\textbf{Fuzz note:} GROUP and UNGROUP expressions emit outside any Z environment — fuzz silently skips them. Do not wrap GROUP/UNGROUP inside \texttt{zed}/\texttt{axdef}/\texttt{schema} blocks.}
