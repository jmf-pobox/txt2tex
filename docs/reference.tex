...
R[B/A]               & $R[B/A]$ — rename (Z RM §3.11, NEW first) \\
R join S             & $\mathrm{Join}(R, S)$ — natural join \\
R join [p] S         & $\mathrm{Join}_{p}(R, S)$ — theta-join \\
R div S              & $R \div S$ — division \\
\end{tabularx}

{\footnotesize \texttt{sigma}/\texttt{pi} bind at atom level. \texttt{R[B/A]} postfix rename requires relational context (inside \texttt{sigma}/\texttt{pi} arg or \texttt{join}/\texttt{div} right operand). \texttt{join} and \texttt{div} are infix at cross-product precedence.}
