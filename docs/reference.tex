...
TEXT: prose here     & Prose (escaped); inline math in \verb|$...$| \\
...
{\footnotesize Inside \kw{TEXT:} prose, wrap whiteboard math in \verb|$...$|. Bare prose is escaped, never converted to symbols.}

\begin{tabularx}{\columnwidth}{LR}
\$forall x : N | P\$ & $\forall x : \nat @ P$ \\
\$A -> B\$           & $A \fun B$ \\
\$x >= 0\$           & $x \geq 0$ \\
\$p.a |-> p.b\$      & $p.a \mapsto p.b$ \\
\$|->\$ / \$forall\$ & $\mapsto$ / $\forall$ (bare symbol) \\
\$A \textbackslash{} B\$ & $A \setminus B$ (set difference; space required) \\
\end{tabularx}

{\footnotesize\textbf{Strict:} raw \LaTeX\ (e.g.\ \texttt{\textbackslash geq}) inside \texttt{\$...\$} is an error --- use a \kw{LATEX:} block instead.}
...
{\footnotesize None of the keywords below can be used as an identifier name or carry decorations (watch the short ones: \texttt{id}, \texttt{dom}, \texttt{P}, \texttt{F}, \texttt{as}) --- except that the relational operators \texttt{group}, \texttt{ungroup}, and \texttt{extend} are still accepted as attribute names in relational-algebra contexts.}
...
union, intersect, setminus, cross, bigcup, bigcap & set operations \\
...
sigma, pi, join, div, group, ungroup, extend & relational algebra \\
...
R cross S            & $R \cross S$ — Cartesian product \\
\end{tabularx}
...
{\scriptsize The aggregator keywords are \kw{Count}, \kw{Sum}, \kw{Avg}, \kw{Min}, \kw{Max}, \kw{Median}. The single-argument form \texttt{Agg(x)} groups by the complement; the two-argument form \texttt{Agg(rel, attr)} aggregates \texttt{attr} over a relation-valued attribute (used with \kw{extend}). The regroup form (\texttt{\{...\} as alias}) and the aggregator form are mutually exclusive in a single \texttt{group} expression.}
...
{\scriptsize Multiple aggregators are comma-separated. Each accepts one attribute identifier, or two for the \texttt{Agg(rel, attr)} form — no nested aggregators, no compound expressions.}

\smallskip
\needspace{10\baselineskip}
\textbf{EXTEND \& two-argument aggregate.}

{\scriptsize \kw{extend} adds a per-tuple computed attribute (Date's EXTEND). The two-argument aggregate \texttt{Agg(rel, attr)} aggregates \texttt{attr} over a relation-valued attribute produced by \texttt{group (\{...\} as rel)}. GROUP nests and reduces cardinality; EXTEND preserves it.}

\begin{tabularx}{\columnwidth}{LR}
R extend (Sum(p, a) as t) & $R \mathop{\mathrm{Extend}}(\mathrm{Sum}(p, a)~\mathrm{as}~t)$ \\
\end{tabularx}
...
\begin{exget}
\begin{center}
$Nested~\mathop{\mathrm{Extend}}(\mathrm{Sum}(payments, amountPaid)$\\
$~\mathrm{as}~totalPaid)$
\end{center}
\end{exget}
...
6  & range \texttt{..} \\
7  & \texttt{intersect}, \texttt{setminus} (\texttt{\textbackslash}) \\
8  & \texttt{cross}, \texttt{join}, \texttt{div}, \texttt{group}, \texttt{ungroup} (cross-product / relational) \\
9  & \texttt{union}, \texttt{++} (override) \\
10 & relational infix \texttt{|->}, \texttt{<|}, \texttt{|>}, \texttt{<<|}, \texttt{|>>}, \texttt{o9}, \texttt{comp} \\
11 & function / relation arrows \texttt{->}, \texttt{<->}, \texttt{+->}, \texttt{>->}, \texttt{>+>}, \texttt{-|>}, \texttt{>->>}, \texttt{-->>}, \texttt{+->>}, \texttt{77->} \\
12 & \texttt{elem}, \texttt{notin}, \texttt{subset}, \texttt{subseteq}, \texttt{psubset} \\
13 & comparison \texttt{<}, \texttt{>}, \texttt{<=}, \texttt{>=}, \texttt{=}, \texttt{!=} \\
14 & \texttt{lnot} \\
15 & \texttt{land} \\
16 & \texttt{lor} \\
17 & \texttt{=>} (right-associative) \\
18 & \texttt{<=>} (left-associative; logically commutative) \\
19 & quantifiers \texttt{forall}, \texttt{exists}, \texttt{exists1}, \texttt{mu}, \texttt{lambda} (loosest) \\
\end{tabularx}

{\scriptsize Schema-calculus operators appear only on the RHS of \kw{defs}; they bind looser than expression-level operators and aren't part of this table.  Internal order, tightest to loosest: \kw{hide} / \kw{project} (postfix) $<$ \texttt{;} composition $<$ \texttt{>{}>} piping.}
