...
mod, div                                   & arithmetic / cardinality \\
...
sigma, pi, join, group, ungroup            & relational algebra \\
...
schema PromoteBump
  ...
  id? : ID
where
  counters(id?) = theta Counter
  counters' = counters [+] {id? |-> theta Counter'}
end
\end{verbatim}
\end{exwrite}
\begin{exget}%
...
id? : ID
\where
counters~id? = \theta Counter \\
counters' = counters \oplus \{id? \mapsto \theta Counter'\}
\end{schema}
\end{exget}
...
3  & function application \texttt{f(x)} \\
4  & relational rename \texttt{R[B/A]} (postfix) \\
5  & relational atoms \texttt{sigma[..]}, \texttt{pi[..]} (prefix) \\
6  & \texttt{*}, \texttt{div}, \texttt{mod} \\
7  & \texttt{+}, \texttt{-} (binary) \\
8  & range \texttt{..} \\
9  & \texttt{cross}, \texttt{join}, \texttt{div} (cross-product) \\
10 & \texttt{union}, \texttt{setminus} (\texttt{\textbackslash}) \\
11 & \texttt{intersect} \\
12 & relational \texttt{<->}, \texttt{|->}, \texttt{<|}, \texttt{|>}, \texttt{<<|}, \texttt{|>>}, \texttt{o9}, \texttt{comp}, \texttt{;} \\
13 & function arrows \texttt{->}, \texttt{+->}, \texttt{>->}, \texttt{>->>}, \texttt{-->>}, \texttt{+->>}, \texttt{77->} \\
14 & \texttt{elem}, \texttt{notin}, \texttt{subset}, \texttt{subseteq}, \texttt{psubset} \\
15 & comparison \texttt{<}, \texttt{>}, \texttt{<=}, \texttt{>=}, \texttt{=}, \texttt{!=} \\
16 & \texttt{lnot} \\
17 & \texttt{land} \\
18 & \texttt{lor} \\
19 & \texttt{=>} (right-associative) \\
20 & \texttt{<=>} (right-associative) \\
21 & quantifiers \texttt{forall}, \texttt{exists}, \texttt{exists1}, \texttt{mu}, \texttt{lambda} (loosest) \\
\end{tabularx}
