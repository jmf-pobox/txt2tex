...
\noindent identity[N] instantiates the generic identity function for
natural numbers.
...
\begin{axdef}
  matrix : \seq (\seq \nat)
  \where
  matrix = \langle \langle 1, 2, 3 \rangle, \langle 4, 5, 6 \rangle,
  \langle 7, 8, 9 \rangle \rangle
\end{axdef}
...
\noindent invalidSeq = ⟨1, -1, 2⟩  -- would be type error $if -1 \notin N$
...
\section*{Example 20 : Instantiation in Quantifiers}
\addcontentsline{toc}{section}{Example 20 : Instantiation in Quantifiers}
...
\begin{axdef}
  ...
  mapping : \nat \rel \nat
  \where
  lookup = \{1 \mapsto \langle 10, 20 \rangle, 2 \mapsto \langle 30,
  40 \rangle\} \\
...
\noindent 2. Use consistent naming $(\seq T, \lnot (mixed(seq)(T))
\land T(seq))$
...
\noindent - seq[N] = ⟨1, 2, 'a'⟩ -- 'a' is $\lnot a$ natural

\bigskip

\noindent - P[N] = $\{1, 2, -1\}$ -- -1 might not $be \in N$
depending on N definition
