...
\noindent This groups related variable declarations in a single block
(given types must be declared outside).
...
\noindent Abbreviations can be defined standalone (zed blocks for
abbreviations may not be fully supported yet).
...
\section*{Example 4 : Predicates in Zed Block}
\addcontentsline{toc}{section}{Example 4 : Predicates in Zed Block}
...
\begin{zed}Color ::= red | green | blue
\end{zed}

\begin{zed}Size ::= small | medium | large
\end{zed}
...
\noindent Multiple schemas can be defined separately, each with their
own constraints.
...
\noindent Schema Address is used within schema PersonData. Variables
using schemas go in axdef.
...
\begin{zed}[UserID, DocumentID]
\end{zed}

\begin{zed}PermissionType ::= read | write | admin
\end{zed}
