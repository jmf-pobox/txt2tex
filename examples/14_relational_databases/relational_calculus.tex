...
\hypersetup{pdftitle={Relational Calculus --- Music Library},pdfauthor={txt2tex example},pdfcreator={txt2tex v2.2.0}}
\begin{document}
...
\addcontentsline{toc}{section}{Query 1: Single-relation selection}
...
\t1 \lblot~title == t.title, genre == t.genre~\rblot ~\}
\end{array}$

\bigskip

\section*{Query 2: Two-relation join via comprehension}
\addcontentsline{toc}{section}{Query 2: Two-relation join via comprehension}
...
\t1 al.arid = ar.arid @ \\
\t1 \lblot~track == t.title, artist == ar.name~\rblot ~\}
\end{array}$

\bigskip

\section*{Query 3: Three-relation chain with playlist}
\addcontentsline{toc}{section}{Query 3: Three-relation chain with playlist}
...
\t1 t.alid = al.alid @ \\
\t1 \lblot~playlist == p, track == t.title, album == al.title~\rblot ~\}
\end{array}$

\bigskip

\section*{Query 4: Binding output, filtered by duration}
\addcontentsline{toc}{section}{Query 4: Binding output, filtered by duration}
...
\t1 t.duration > 300 @ \\
\t1 \lblot~track == t.title, album == al.title~\rblot ~\}
\end{array}$

\bigskip

\section*{Query 5: Multi-decl forall}
\addcontentsline{toc}{section}{Query 5: Multi-decl forall}
...
\addcontentsline{toc}{section}{Query 6: Multi-decl comprehension, no characteristic expression}
