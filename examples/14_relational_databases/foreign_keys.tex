...
\noindent Each relation has exactly one pk line.  txt2tex underlines the attribute

\bigskip

\noindent in the schema box and emits a PK statement immediately after.
...
\noindent Two pk lines in one schema body.  Both attributes are underlined;

\bigskip

\noindent txt2tex collapses them into one PK $set : PK(RecipeIngredient)$ = $\{recipeId, ingredientId\}$.
...
\noindent A foreign key is a standalone Z predicate.  The binding-expression form maps

\bigskip

\noindent one source attribute to one target attribute:

\bigskip

\noindent (SourceRelation, TargetRelation) $\mapsto$ $\{srcAttr \mapsto tgtAttr\}$ $\in$ FK
...
\noindent Both FK predicates for RecipeIngredient can be stated together.

\bigskip

\noindent Each is a separate standalone predicate.
...
\noindent Employee carries a managerId that references another row in the same relation.

\bigskip

\noindent The self-referential FK predicate uses FKEE, declared above.
