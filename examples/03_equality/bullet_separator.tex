...
\noindent This reads: "the set of x squared, for all $x \in N$ where
x is positive". The constraint $x > 0$ filters the domain, and x * x
...
\noindent NOTE: Full bullet support in $\forall$/exists quantifiers
is not yet implemented (see STATUS.md). Currently use implication instead:
...
\noindent Before $bullet : constraint$ $(declaration \land filter)$
