...
\noindent Finds $n = 11$ $(unique(n)(between)(10) \land 12)$, then
returns 2 * 11 = 22.
...
\begin{zed}[ConfigKey, ConfigValue]
\end{zed}
...
\begin{axdef}
  ...
  \where
  \forall k : ConfigKey @ \\
  \quad k \in \dom configs \implies \\
  \quad getConfigInt(k) = (\mu v : \nat | parseValue(configs(k)) = v)
\end{axdef}
...
\begin{axdef}
  ...
  \implies \\
  \quad getConfigWithDefault(k, default) = (\mu v : \nat |
  parseValue(configs(k)) = v) \\
  \forall k : ConfigKey @ \forall default : \nat @ k \notin \dom
  configs \implies \\
  \quad getConfigWithDefault(k, default) = default
\end{axdef}
...
\addcontentsline{toc}{section}{Example 11 : Comparison - With and
Without Bullet}
...
\noindent $(\mu x : \nat | x * x =
-(1(no(natural)(number)(has)(negative)(square))))$
