...
\noindent Given a primitive relation childOf between people, define derived relations

\bigskip

\noindent for parentOf, siblingOf, cousinOf, and ancestorOf using relation operators.
...
\noindent From this single primitive, we can derive all other family relationships.
...
\noindent parentOf == $\{ x, y \colon Person \mid x \mapsto y \in childOf \bullet y \mapsto x \}$
...
\noindent All three methods are mathematically equivalent. The first is most idiomatic
...
\noindent If (x, p) in childOf and (p, y) in parentOf, then (x, y) is in the composition.

\bigskip

\noindent This includes the case where $x = y$ (you share parents with yourself).
...
\noindent Step 3: (childOf $\circ$ parentOf) \ id[Person]
...
\noindent siblingOf == {x, $y : Person | x $/= y and

\bigskip

\noindent ($\exists p \colon Person \bullet x \mapsto p$in childOf and
...
\noindent - siblingOf is symmetric: if x is sibling of y, then y is sibling of x
...
\noindent - For second cousins, you would need (childOf $\circ$ siblingOf $\circ$ parentOf)+
...
\noindent - Includes great-grandparents (parentOf $\circ$ parentOf $\circ$ parentOf)
...
\noindent 1. **Primitive vs derived**: Start with minimal primitives, derive everything else
...
\noindent 2. **Relation algebra**: Use operators instead of explicit set comprehensions
...
\noindent 4. **Operator chaining**: Compose multiple operators for complex relationships
...
\noindent 2. Define grandchildOf two $ways : using composition and using descendantOf$
...
\noindent 4. What properties must childOf satisfy for this model to be realistic?
