...
\noindent For each person p, children(p) is the set of all people c where p is a parent of c.
...
\noindent Recall: R(| A |) = $\{ y \mid \exists x \colon A \bullet x \mapsto y \in R \}$
...
\noindent $So : parentOf(| $$\{p\}$ |) = $\{ c \mid p \mapsto c \in parentOf \}$
...
\noindent children = $\{ p \colon Person \bullet p \mapsto \{ c \colon Person \mid p \mapsto c \in parentOf \} \}$
...
\noindent The relation image version is more concise and $idiomatic \in Z$ notation.
...
\addcontentsline{toc}{section}{Solution ( b ) : Number of Grandchildren Function}
...
\noindent Type $signature : number_of_grandchildren $: $Person \fun \mathbb{N}$
...
\noindent Grandchildren are children of children, which we can express via composition.
...
\begin{axdef}
\mathit{children\_alt} : Person \fun \power Person
\where
\forall p \colon Person \bullet \mathit{children\_alt}(p) = \{ c \colon Person \mid p \mapsto c \in parentOf \}
\end{axdef}

\begin{axdef}
\mathit{number\_of\_grandchildren\_alt} : Person \fun \mathbb{N}
\where
\forall p \colon Person \bullet \mathit{number\_of\_grandchildren\_alt}(p) = \# \{ g \colon Person \mid \exists c \colon Person \bullet p \mapsto c \in parentOf \land c \mapsto g \in parentOf \}
\end{axdef}
...
\where
\forall p \colon Person \bullet grandchildren(p) = \bigcup \{ c \colon children(p) \bullet children(c) \}
\end{axdef}

\begin{axdef}
\mathit{number\_of\_grandchildren\_v3} : Person \fun \mathbb{N}
\where
\forall p \colon Person \bullet \mathit{number\_of\_grandchildren\_v3}(p) = \# grandchildren(p)
\end{axdef}
...
\noindent - children: $Person \fun \power$ Person (a person may have 0+ children)
...
\noindent 2. **Relation image for set operations**: Use (| ... |) instead of comprehensions

\bigskip

\noindent - More $concise : R(| $$\{x\}$ |) vs $\{ y \mid x \mapsto y \in R \}$
...
\noindent 3. **Composition for multi-step relationships**: Use $\circ$ instead of nested quantifiers
...
\noindent 5. **Progressive abstraction**: Build complex functions from simpler ones
...
\where
children = \{ p \colon Person \bullet p \mapsto (parentOf \limg \{p\} \rimg) \} \\
grandchildren = \{ p \colon Person \bullet p \mapsto ((parentOf \circ parentOf) \limg \{p\} \rimg) \} \\
\mathit{number\_of\_grandchildren} = \{ p \colon Person \bullet p \mapsto \# grandchildren(p) \}
\end{axdef}
...
\noindent 2. Define a function $\mathit{oldest\_child} : Person  \pfun  Person$
...
\noindent 3. Prove that: # grandchildren(p) = sum {# children(c) $\mid$ c in children(p)}
...
\noindent - Consistent cardinality (if p in children(x), then x in parentOf(| $\{p\}$ |))
