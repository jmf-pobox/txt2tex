...
\begin{axdef}
  ...
  \where
  \forall p : Person @ \\
  \quad number\_of\_grandchildren\_alt(p) = \# \{ g : Person |
    \exists c : Person @ p \mapsto c \in parentOf \land c \mapsto g \in
  parentOf \}
\end{axdef}
...
\begin{axdef}
  ...
  \where
  \forall p : Person @ \\
  \quad grandchildren(p) = \bigcup \{ c : children(p) @ children(c) \}
\end{axdef}
...
\noindent $Definition : number_of_grandchildren$ = $\{ p : Person @ p
