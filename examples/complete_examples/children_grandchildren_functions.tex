...
\noindent Recall: $(R \limg A \rimg)$ = {y $\mid$ $\exists x : A @ x
\mapsto y$in R}
...
\noindent So: $(parentOf \limg \{p\} \rimg)$ = {c $\mid$ p $\mapsto$
c in parentOf}
...
\noindent children = {$p : Person$ . p $\mapsto$ {$c : Person$ $\mid$
p $\mapsto$ c in parentOf}}
...
\noindent The relation image version is more concise and idiomatic in
Z notation.
...
\noindent - More concise: $(R \limg \{x\} \rimg)$ vs {y $\mid$ x
$\mapsto$ y in R}
...
\noindent Definition: number\_of\_grandchildren = $\{~ p : Person @ p
\mapsto \# (grandchildren(p)) ~\}$
