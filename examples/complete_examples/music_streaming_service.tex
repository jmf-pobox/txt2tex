...
\begin{axdef}
  ...
  \subseteq users \\
  \forall i : \dom playlists @ playlistOwner(i) \\
  in(playlistSubscribers(i))
\end{axdef}
...
\begin{zed}
  ...
  u : UserId | s \in loved(u) ~\} > \# \{~ u : UserId | t \in loved(u) ~\}))
\end{zed}
...
\noindent - # {$u : UserId$ $\mid$ s in loved(u)}: count users who love song s
...
\noindent - $>$ # {$u : UserId$ $\mid$ t in loved(u)}: s has more lovers than t
...
\begin{axdef}
  ...
  \where
  \forall i : songs @ loveHateScore(i) = \IF \# \{~ u : UserId | i
  \in loved(u) ~\} \geq \# \{~ u : UserId | i \in hated(u) ~\} \THEN
  \# \{~ u : UserId | i \in loved(u) ~\} - \# \{~ u : UserId | i \in
  hated(u) ~\} \ELSE 0
\end{axdef}
