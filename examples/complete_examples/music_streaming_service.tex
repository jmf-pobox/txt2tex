...
\noindent or Apple Music. The system manages songs, users, playlists, preferences,
...
\noindent F means finite $\lnot infinite$.
...
\noindent 1. $\forall i \colon \dom playlists \bullet \ran playlists(i) \subseteq songs$
...
\noindent 5. $\forall i \colon \dom playlistSubscribers \bullet playlistSubscribers(i) \subseteq users$
...
\noindent 6. $\forall i \colon \dom playlists \bullet playlistOwner(i)$in playlistSubscribers(i)
...
\noindent Model users' song preferences $(loved \land hated(songs))$:
...
\noindent 2. $\forall i \colon \dom hated \bullet hated(i) \subseteq songs$
...
\noindent 4. $\forall i \colon \dom loved \bullet loved(i) \subseteq songs$
...
\noindent 5. $\forall i \colon \dom hated \cup \dom loved \bullet hated(i) \cap loved(i) = \emptyset$

\bigskip

\noindent **Critical consistency constraint**: A user cannot love and hate the same song.

\bigskip

\noindent For every user who has expressed preferences, their loved and hated sets are disjoint.
...
\noindent - ran(playlistSubscribers): all subscriber sets for all playlists

\bigskip

\noindent - bigcup(...): union of all these $sets = all$ users subscribed to anything
...
\noindent - $p : dom(playlistSubscribers)$: iterate over all playlists with subscribers
...
\noindent - $\forall v \colon \dom loved \bullet u \neq v$: for all other users who love songs
...
\noindent - # $\{ u \colon UserId \mid s \in loved(u) \}$: count users who love song s
...
\noindent - $>$ # $\{ u \colon UserId \mid t \in loved(u) \}$: s has more lovers than t
...
\noindent Complexity: For each song, counts how many users love it, then finds unique max.
...
\noindent - If loves $\geq$ $hates : score $= loves - hates (net positive sentiment)
...
\where
\forall i \colon songs \bullet playlistCount(i) = \# \{ p \colon \dom playlists \mid i \in \ran playlists(p) \}
\end{axdef}
...
\noindent popularity(song) = (loves - hates, capped at 0) + (number of playlists)
...
\noindent - $\exists_1$: check exactly one song has unique maximum popularity

\bigskip

\noindent - If unique max exists: use mu to find it

\bigskip

\noindent - If no unique max (tie): return nullSong (error value)
...
\noindent This prevents undefined behavior when multiple songs tie for most popular.
...
\begin{zed}[Play, Status]\end{zed}

\noindent Assume Play is a $schema : schema Play ${ $songId : SongId, userId$: UserId, $timestamp : Time $}

\bigskip

\noindent Assume Status is a free $type : Status $::= premium $\mid$ standard

\bigskip

\noindent Define functions to$ \filter $and analyze play history:

\bigskip

\subsection*{(a)}

\noindent Extract plays by premium users:
...
\noindent Recursive pattern matching:

\bigskip

\noindent - Base $case : empty sequence yields empty sequence$

\bigskip

\noindent - Inductive case: if first play x is by premium user, include it; else skip

\bigskip

\noindent - x.2 accesses the userId field (assuming Play is a tuple or schema)

\bigskip

\noindent - userStatus is assumed to be a $function : UserId  \fun  Status$
...
\noindent Extract plays by standard users:
...
\noindent Similar to premiumPlays, but filters for standard users.
...
\noindent Calculate cumulative listening time:
...
\noindent Fold (reduce) pattern:

\bigskip

\noindent - Base case: empty sequence has zero length

\bigskip

\noindent - Inductive $case : add first play$'s length to rest's total

\bigskip

\noindent - x.1 accesses the songId field

\bigskip

\noindent - length(songId) returns song duration (from earlier definition)
...
\noindent Usage: Total listening time for a user or for entire service.
...
\noindent schema Play { $songId : SongId, userId$: UserId, $timestamp : Time $}
...
\noindent 1. **Multi-entity modeling**: songs, users, playlists, plays work together
