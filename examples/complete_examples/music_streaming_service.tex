...
\noindent - # $\{ u : UserId | s \in loved(u) \}$: count users who love song s
...
\noindent - $>$ # $\{ u : UserId | t \in loved(u) \}$: s has more lovers than t
...
\begin{axdef}
  ...
  \where
  \forall i : songs @ \\
  \quad playlistCount(i) = \# \{ p : \dom playlists | i \in \ran
  (playlists(p)) \}
\end{axdef}
...
\begin{axdef}
  ...
  \forall i : songs @ \\
  \quad popularity(i) = loveHateScore(i) + playlistCount(i)
\end{axdef}
...
\begin{zed}Status ::= premium | standard
\end{zed}
...
\noindent $premiumPlays : seq(PlayEvent)$ $\fun$ seq(PlayEvent)

\bigskip

\noindent premiumPlays($\langle \rangle$) = $\langle \rangle$

\bigskip

\noindent $\forall$ $x : PlayEvent; s$ : seq(PlayEvent) $\mid$

\bigskip

\noindent premiumPlays($\langle x \rangle$ ^ s) = if
userStatus(x.userId) = premium

\bigskip

\noindent then $\langle x \rangle$ ^ premiumPlays(s)

\bigskip

\noindent else premiumPlays(s)
...
\noindent - Inductive $case : if first play x$ is by premium user,
include it; else skip
...
\noindent $standardPlays : seq(PlayEvent)$ $\fun$ seq(PlayEvent)

\bigskip

\noindent standardPlays($\langle \rangle$) = $\langle \rangle$
...
\noindent standardPlays($\langle x \rangle$ ^ s) = if
userStatus(x.userId) = standard

\bigskip

\noindent then $\langle x \rangle$ ^ standardPlays(s)

\bigskip

\noindent else standardPlays(s)
...
\noindent $cumulativeLength : seq(Play)$ $\fun$ N

\bigskip

\noindent cumulativeLength($\langle \rangle$) = 0

\bigskip

\noindent $\forall$ $x : Play; s$ : seq(Play) $\mid$

\bigskip

\noindent cumulativeLength($\langle x \rangle$ ^ s) =
songLength(x.songId) + cumulativeLength(s)
...
\noindent - Inductive case: add first play's song length to rest's total

\bigskip

\noindent - x.songId accesses the songId field (if Play were a schema)

\bigskip

\noindent - songLength(songId) would return song duration from the catalog
