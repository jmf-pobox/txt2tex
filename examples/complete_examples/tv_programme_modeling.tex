...
\begin{axdef}
  ...
  \t1 removeTheLongestViewed(s) = s \filter \{~ x : \ran s | x \neq
  longestViewed(s) ~\}
\end{axdef}
...
\noindent Declarative specification (defines properties, not algorithm):
...
\noindent Specification approach (not implementation):
...
\noindent ($\forall p : \ran s @ p.length > 0(and)(p.length)$$\leq$ 360) and
