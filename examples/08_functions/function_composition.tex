...
\noindent Forward composition f $\semi$ g:

\bigskip

\noindent Input $\rightarrow$ f $\rightarrow$ g $\rightarrow$ Output

\bigskip

\noindent (f $\semi$ g)(x) = g(f(x))
...
\noindent Input $\rightarrow$ g $\rightarrow$ f $\rightarrow$ Output
...
\forall n : \nat @ double(n) = 2 * n \\
\forall n : \nat @ square(n) = n * n \\
pipeline = addOne \semi double \semi square
\end{axdef}
...
\forall n : \nat @ identity(n) = n \\
\forall n : \nat @ anyFunc(n) = n * n \\
leftId = identity \semi anyFunc \\
rightId = anyFunc \semi identity
\end{axdef}

\noindent $leftId = anyFunc$ (identity is left identity for $\semi$)

\bigskip

\noindent $rightId = anyFunc$ (identity is right identity for $\semi$)
...
\forall n : \nat @ g3(n) = n * 2 \\
\forall n : \nat @ h3(n) = n * n \\
comp1 = (f3 \semi g3) \semi h3 \\
comp2 = f3 \semi (g3 \semi h3)
\end{axdef}
...
\where
grandparent = parent \semi parent
\end{axdef}
...
\where
process = toLowerCase \semi trim \semi reverse
\end{axdef}
...
\forall n : \nat @ squareFunc(n) = n * n \\
\forall n : \nat @ tripleFunc(n) = 3 * n \\
combined = squareFunc \semi tripleFunc
\end{axdef}
...
\where
identity\_check = encrypt \semi decrypt
\end{axdef}
...
\noindent 1. Composition is associative: (f $\semi$ g) $\semi$ $h = f$ $\semi$ (g $\semi$ h)

\bigskip

\noindent 2. Identity $element : id(x)$ = x, id $\semi$ $f = f = f$ $\semi$ id
...
\where
\forall n : \nat @ doubleIt(n) = 2 * n \\
quadrupleIt = doubleIt \semi doubleIt
\end{axdef}
...
\where
\forall n : \nat @ increment(n) = n + 1 \\
addFive = increment \semi increment \semi increment \semi increment \semi increment
\end{axdef}
...
\noindent $Use : pipeline$ = f $\semi$ g $\semi$ h; $result = pipeline$(x)
...
\noindent 1. Choose forward ($\semi$) or backward (o) consistently in your codebase
...
\noindent 7. $Remember :  \semi  reads left-to-right$ (natural for pipelines), o reads right-to-left (traditional math notation)
