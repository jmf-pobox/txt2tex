...
\begin{zed}[Person, Char, B, Record, ValidatedRecord, ProcessedRecord, Output]
\end{zed}
...
\noindent Notice the $reversal :  \circ  applies left-to-right$, o
...
\noindent pipeline(n) = square(double(addOne(n))) = square(double(n +
1)) = square(2(n + 1)) = (2(n + 1))^2
...
\noindent composed is only defined where safeSqrt is defined, then
adds 10 to the result.
...
\noindent grandparent relates a person to their grandparents via two
parent links.
...
\noindent Process a string by converting to lowercase, trimming
whitespace, then reversing.
...
\noindent If encrypt and decrypt are inverses, then identity\_check
...
\noindent When composing f : $A \fun B$ and $g : C$ $\fun$ D, we need
...
\noindent h4 = $\{1 \mapsto 100, 2 \mapsto 200\}$. Composition is
defined where ranges align.
...
\noindent 1. Composition is associative: (f $\circ$ g) $\circ$ $h =
f$ $\circ$ (g $\circ$ h)
...
\noindent 3. Closure: composing $A \fun A$ functions gives another $A
\fun A$ function

\bigskip

\section*{Example 14 : Practical Example - Data Processing Pipeline}
\addcontentsline{toc}{section}{Example 14 : Practical Example - Data
Processing Pipeline}

\noindent In practice, you might compose functions to create data
processing $pipelines : validate  \circ  process  \circ  format. Each
stage transforms data through$ a series of operations. The notation f
$\circ$ g $\circ$ h means "apply f, then g, then h" which creates a
...
\noindent f $\circ$ $g = h$ means (f $\circ$ g) = h, $\lnot f$ $\circ$ ($g = h$)
...
\begin{axdef}
  ...
  \where
  \forall n : \nat @ increment(n) = n + 1 \\
  addFive = increment \circ increment \circ increment \circ increment
  \circ increment
\end{axdef}
...
\noindent This separates pipeline definition from application,
...
\noindent 1. Choose forward ($\circ$) or backward (o) consistently in
your codebase
...
\noindent 7. $Remember :  \circ  reads left-to-right$ (natural for
pipelines), o reads right-to-left (traditional math notation)
