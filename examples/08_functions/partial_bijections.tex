...
\begin{axdef}
  ...
  \where
  \# (\dom enrollment) \leq 100 \land (\forall s1, s2 : Student @ s1
    \in \dom enrollment \land s2 \in \dom enrollment \implies (s1 \neq
  s2 \implies enrollment(s1) \neq enrollment(s2)))
\end{axdef}
...
means $X > 7$$\fun$ ($Y \pbij \num$), which is a partial bijection
from X to partial bijections from Y to Z.
...
\begin{axdef}
  ...
  f\_composed : \nat \pbij \nat
  \where
  f\_inv = f^{-1} \land f\_composed = g \circ f
\end{axdef}
