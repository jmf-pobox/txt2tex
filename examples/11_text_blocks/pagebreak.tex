...
\noindent This content appears on the next page. The PAGEBREAK
directive forces a page break at that point in the document.
...
\noindent When preparing solutions for submission, you may want each
...
\noindent Content for section 1 covering propositional logic
...
\noindent For lengthy proofs, you might want to start each major
proof on a new page:
...
\begin{center}
  \adjustbox{max width=\textwidth}{%
    ...
    \infer[\Rightarrow\textrm{-intro}^{[1]}]{p(and(p \implies q)) \implies q}{
      \infer[\Rightarrow
      \mbox{elim}]{q}{\infer[\land\textrm{-elim-1}]{p}{\ulcorner
        p(and(p \implies q)) \urcorner^{[1]}} &
        \infer[\land\textrm{-elim-2}]{p \implies q}{\ulcorner p(and(p
...
\begin{center}
  \adjustbox{max width=\textwidth}{%
    ...
    \infer[\Rightarrow\textrm{-intro}^{[1]}]{p(or)(q)(and(not(q))) \implies p}{
      \infer[\lor
      \mbox{elim}]{p}{\infer[\land\textrm{-elim-2}]{not(q)}{\infer[\land\textrm{-elim-1}]{p(or)(q)}{\ulcorner
      p(or)(q)(and(not(q))) \urcorner^{[1]}}}}
    }
    $%
  }
\end{center}
...
\noindent Use PAGEBREAK strategically to improve document
readability. Don't overuse it—let LaTeX handle most page breaking automatically.
...
\noindent When you have a section title or definition that would
appear at the bottom of a page with its content on the next page, you
might insert a PAGEBREAK before the section.
...
\noindent For coursework submissions, instructors may require one
solution per page:
...
\noindent This format makes it easy for instructors to review and
grade individual problems.
