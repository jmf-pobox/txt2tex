...
exponentiation \citep{spivey1993fuzz}.
...
\noindent We are designing a prototype for a new podcast platform and
player.  We are given some initial types and abbreviations.  We also
define two generic functions fst() and snd() to provide our fuzz
compatible and validated solution capabilities equal to tuple
projection, which is not supported in fuzz \citep{spivey1993fuzz}.
...
\begin{axdef}
  ...
  \quad e \in show\_episodes(s) \iff (s, e) \in \dom episode\_info \\
  shows \subseteq \dom show\_info \land \dom show\_info \subseteq shows \\
  \forall s : shows @ \forall e : EpisodeId @ \forall p : PeopleId @
  p \in fst(episode\_info(s, e)) \iff p \in show\_info(s) \\
  \forall s : shows @ \forall t_1, t_2 : Timestamp @ \forall e_1, e_2
  : EpisodeId @ \\
  \quad (((s, e_1) \in \dom episode\_info \land (s, e_2) \in \dom
    episode\_info) \implies \\
    \quad ((e_1 < e_2) \implies (snd(episode\_info(s, e_1)) \leq
  snd(episode\_info(s, e_2)))))
\end{axdef}

\noindent The above Z is validated using fuzz. Each part is briefly
explained below:
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent If and only if translates to the sets are equal. The same
ShowId exist in both sets.
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent Each finite set of EpisodeId includes an uninterrupted
range from 1 to n.
...
\noindent\hspace*{\parindent}(e) $\forall s : shows @ \forall e :
EpisodeId @ \forall p : PeopleId @ p \in fst(episode\_info(s, e))
\iff p \in show\_info(s)$

\bigskip

\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent The specification "exactly when" translates to
bi-implication and fst(episode\_info(s, e)) gives us access to a
finite set of PeopleId as does show\_info(s) .
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent We specify how the Timestamp for different episodes of the
same show must compare to fulfill the specification by using
snd(episode\_info(s, e)) to access the Timestamp.
...
\noindent The following solutions are expressed as abbreviations to
enable fuzz validation, which passes for each expression.
...
make sure two such shows are distinct.
...
\begin{axdef}
  ...
  \where
  \forall s : shows @ (s, 1) \in \dom episode\_info \implies
...
1$ for $s : ShowId$.  We are given there is exactly one episode
associated with the most people. Therefore, we know that we will call
first\_episode\_timestamp with a ShowId that has at least one episode
and we are guaranteed that $EpisodeId = 1$ exists by constraint 7(b).
In addition, calling this function with the s from the mu expression
is valid because the constraint defined in 7(e) guarantees that the
set of EpisodeId in the domain of show\_info and in the first element
of the domain of episode\_info are identical. Therefore, calling this
partial function is guaranteed to be valid with the mu expression as its input.
...
\noindent We are now modeling an individual podcast player from the
point of view of a user.  N.B. The free types PlayedOrNot and
SavedOrNot have been adjusted to avoid ambiguity with propositions P
and S. The following specification has been validated with fuzz.
...
\begin{axdef}
  ...
  my\_episodes \implies \\
  \quad e \in show\_episodes(s) \\
  \forall s : ShowId @ \forall e : EpisodeId @ (s, e) \in \ran
  my\_episodes \implies \\
  \quad s \in my\_shows \\
  \forall s : ShowId @ \forall e : EpisodeId @ \\
  \quad (s, e) \in \ran my\_episodes \iff (s, e) \in \dom status
\end{axdef}

\noindent\hspace*{\parindent}(a) $my\_shows \subseteq shows$

\bigskip

\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent The elements of the finite set of ShowId on this device
must be elements of shows
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent We use the ran operator to access the (s, e) in the
injective seq my\_episodes to specifiy each pair just be a valid
mapping in show\_episodes
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent We use the ran operator to access the (s, e) in the
injective seq my\_episodes to specifify each ShowId must be in my\_shows
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent Every (s, e) pair in the ran my\_episodes has a status and
every (s, e) in status must be in ran my\_episodes
...
\noindent Since the base case is proven and both inductive cases
result in the same outcome, we can use the implication introduction
rule [$\Rightarrow$ intro] to unify our cases (n.b. Length\_L is
abbreviated L below to spare the reader further font scaling) and conclude:
