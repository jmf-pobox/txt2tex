...
Given a relation $R \in X$ $\rel$ Y and a second relation $S \in Y$ $\rel$ Z, and two elements $x \in X$ and $z \in Z$, x $\mapsto$ $z \in R$ o9 S $\Leftrightarrow$ $\exists y : Y @ x \mapsto y \in R \land y \mapsto z \in S$ \citep[page 107]{simpson-notes}.
...
&\Leftrightarrow \exists y : Y @ w \mapsto y \in (\exists x : X @ w \mapsto x \in R \land x \mapsto y \in S) \land y \mapsto z \in T & [\mbox{definition of $\circ$}] \\
&\Leftrightarrow \exists y : Y @ (\exists x : X @ w \mapsto x \in R \land x \mapsto y \in S) \land y \mapsto z \in T & [\mbox{definition of $\circ$}] \\
&\Leftrightarrow \exists y : Y @ \exists x : X @ w \mapsto x \in R \land x \mapsto y \in S \land y \mapsto z \in T & [\mbox{x is $\lnot$ free in y $\mapsto$ z}] \\
&\Leftrightarrow \exists x : X @ w \mapsto x \in R \land x \mapsto z \in (\exists y : Y @ x \mapsto y \in S \land y \mapsto z \in T) & [\mbox{y is $\lnot$ free in w $\mapsto$ x , x $\mapsto$ z}] \\
...
In step 1, we apply the composition equivalence based on the parenthesis, replacing the outer composition of (R o9 S) o9 T. In step 2, we apply our equivlence for composition to (R o9 S).
...
\begin{gendef}[X]
  count\_s: \seq X \fun \nat
\where
  count\_s(\langle \rangle) = 0 \\
  \forall x : X @ \forall s : \seq X @ count\_s(\langle x \rangle \cat s) = 1 + count\_s(s)
\end{gendef}

\begin{gendef}[X]
  reverseSeq: \seq X \fun \seq X
\where
  reverseSeq(\langle \rangle) = \langle \rangle \\
  \forall x : X @ \forall s : \seq X @ reverseSeq(\langle x \rangle \cat s) = reverseSeq(s) \cat \langle x \rangle
\end{gendef}
...
\bibitem[Simpson, 2025]{simpson-notes} Simpson, A. (n.d.). \textit{From Discrete Mathematics to State-Based Models, SEM version}. University of Oxford Department of Computer Science. Unpublished course notes.
