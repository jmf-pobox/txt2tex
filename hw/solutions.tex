...
projection, which is not supported in fuzz.
...
\begin{array}{l}
  \forall s : \dom show\_episodes @ \forall e : show\_episodes(s) @ \\
  \quad e \in show\_episode(s) \iff (s, e) \in \dom episode\_info
\end{array}$
...
\setlength{\leftskip}{\dimexpr\parindent+2em\relax}
\noindent The specification "exactly when" translates to if and only
if and fst(episode\_info(s, e)) gives us access to a finite set of
PeopleId as does show\_info(s) .
