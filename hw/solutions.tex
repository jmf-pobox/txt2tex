...
We are defining a function that will tell us the difference in cardinality between the length of the longest and shorest sequences from an input type that is a non-empty sequences of sequences. The input could be $<$ $\langle x, y, z \rangle$, $\langle \rangle$ $>$ or $<$ $\langle x \rangle$ $>$ for example. I specify three generic functions: maxSeq returns the longest sequence from a non-empty sequence of sequences; minSeq returns the shortest sequence from a non-empty sequence of sequences; and maxDiff, which fulfills the specification for Question 4.

\bigskip

\bigskip

For each function, a mu function is defined that selects a single element meeting the specification based on comparing each sequence s with every sequence t. This is enabled with the ran range operator so that we can go from $ss < 1$ $\mapsto$ $\langle x, y, z \rangle$, 2 $\mapsto$ $\langle \rangle$ $>$ as the data structure to s, t $\langle x, y, z \rangle$, $\langle \rangle$ . maxSeq and minSeq use the cardinality operator and maxDiff uses subtraction with the condition that maxSeq must already $be \geq minDiff$ and therefore the subtraction is guaranteed to return a natural number as defined in the specification. This solution passes fuzz validation.
