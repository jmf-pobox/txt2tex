...
\infer[\Rightarrowintro \mathrm{from} 1]{p \implies (\lnot p \implies p)}{
  ...
  p}{\ulcorner p \urcorner^{[1]} &
    \infer[false\textrm{-elim}^{[3]}]{\lnot
    p}{\infer[\mathrm{false}-\mathrm{intro}]{false}{\ulcorner \lnot p
  \urcorner^{[2]} & \ulcorner p \urcorner^{[3]}}}}
}
$
...
\begin{axdef}
  ...
  \quad (\forall pl : PossiblePlaylist @ \forall ple : (\dom status
    \cross \nat) @ \\
    \quad (fst(status(fst(ple))) = Played \land unplayed(\langle ple
    \rangle \cat pl) = unplayed(pl)) \land \\
    \quad unplayed(\langle ple \rangle \cat pl) = \langle ple \rangle
  \cat unplayed(pl))
\end{axdef}
...
\begin{axdef}
  ...
  \quad (\forall pl : PossiblePlaylist @ \forall ple : (\dom status
    \cross \nat) @ \\
    \quad (fst(status(fst(ple))) = NotP \land played(\langle ple
    \rangle \cat pl) = played(pl)) \land \\
    \quad played(\langle ple \rangle \cat pl) = \langle ple \rangle
  \cat played(pl))
\end{axdef}
