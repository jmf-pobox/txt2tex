...
$p \implies (\lnot p \implies p)$ is a tautology as it is true for
every interpretation of p as illustrated by the final column of truth values.
...
The statements $(\lnot p \implies \lnot q)$, $(q \implies p)$ are
equivalent and $(\lnot p \implies \lnot q) \iff (q \implies p)$ is a
tautology as it is true for every interpretation of p, q as
...
The statements $((p \implies r) \land (q \implies r))$, $((p \lor q)
\implies r)$ are equivalent and $((p \implies r) \land (q \implies
r))$ $\Leftrightarrow$ $((p \lor q) \implies r)$ is a tautology as it
is true for every interpretation of p, q, r as illustrated by the
final column of truth values.
...
In this equivalence proof, we show that $p \implies (\lnot p \implies
p)$ by using implication disjunction to replace the implication with
a logical or, the definition of double negation to simplify, the
application of the idempotence rule to simplify, and the fact that $p
\implies p$ is a tautology.
...
In this equivalance proof, we show that $(\lnot p \implies \lnot q)
\iff q \implies p$ by using implication disjunction to replace
implication with logical or, the definition of double negation to
simplify, the commutivity property of logical or to change the order
of the elements, and the implication disjunction rule to reintroduce
implication.
...
In this equivalence proof, we show that $((p \implies r) \land (q
\implies r))$ $\Leftrightarrow$ $(p \lor q)$ $\Rightarrow$ r by using
the implication disjunction rule to replace implication with logical
or, the commutativity of or rule to change the order of the elements,
the distributivity 2 rule to extract r, and the commutivity of
logical or to change the order of the elements, De Morgan to extract
negation for $(\lnot p \land \lnot q)$, and the implication
...
\noindent
$\displaystyle
\infer[\Rightarrowintro \mathrm{from} 1]{p \implies (\lnot p \implies p)}{
  \ulcorner p \urcorner^{[1]}
  &
  \infer[\Rightarrowintro \mathrm{from} 2]{\lnot p \implies
...
\implies p)$, which we know holds given the truth table in 1(a).
...
$(\lnot p \implies p)$.  This statement requires that I have both p
and $\lnot p$ as premisses, which is a bit odd and implied I needed
...
to show $p \implies q$ and $q \implies p$ given this is the
definition of $p \iff q$.

\bigskip

\bigskip

In one direction, we are seeking to prove $(\lnot p \implies \lnot
q)$ $\Rightarrow$ $(q \implies p)$:

\bigskip

\noindent
$\displaystyle
\infer[\Rightarrowintro \mathrm{from} 1]{(\lnot p \implies \lnot q)
\implies (q \implies p)}{
  \ulcorner \lnot p \implies \lnot q \urcorner^{[1]}
  &
  \infer[\Rightarrowintro \mathrm{from} 2]{q \implies p}{
    \ulcorner q \urcorner^{[2]}
    &
    \infer[false\textrm{-elim}^{[3]}]{p}{
      \infer[\mathrm{false} - \mathrm{intro}]{false}{
        \infer[\Rightarrowelim]{\lnot q}{
          \ulcorner \lnot p \implies \lnot q \urcorner^{[1]}
          &
...
me $q \implies p$ as the key premise to build up, which requires
...
and in the other, we are seeking to prove $(q \implies p)$
...
\noindent
$\displaystyle
\infer[\Rightarrowintro \mathrm{from} 1]{(q \implies p) \implies
(\lnot p \implies \lnot q)}{
  \ulcorner q \implies p \urcorner^{[1]}
  &
  \infer[\Rightarrowintro \mathrm{from} 2]{\lnot p \implies \lnot q}{
    \ulcorner \lnot p \urcorner^{[2]}
    &
    \infer[false\textrm{-elim}^{[3]}]{\lnot q}{
      \infer[\mathrm{false} - \mathrm{intro}]{false}{
        \infer[\Rightarrowelim]{p}{
          \ulcorner q \implies p \urcorner^{[1]}
          &
...
$q \implies p$ as assumption [1] to gain the conclusion.  I need
$(\lnot p \implies \lnot q)$, which shows me to use the implication
rule again with $\lnot p$ as assumption [2].  From there, the
question is how to get $\lnot q$.  Now working from the top, given
assumption [1] has q, it makes sense to use the implication elimation
rule with q as assumption [3]. This gives me p, when what I want is
$\lnot q$.  I see that if I introduce $\lnot p$ as assumption [3]
that I can negate q via the false-intro and false-elim rules and
...
This proof came more quickly because $q \implies p$ as assumption [1]
gave clear direction for the top portion. All assumptions are
...
\noindent
$\displaystyle
\infer[\Leftrightarrowintro]{(\lnot p \implies \lnot q) \iff (q \implies p)}{
...
In one direction, we are seeking to prove $(p \implies r)$ and $(q
\implies r)$ $\Rightarrow$ $(p \lor q \implies r)$:

\bigskip

\noindent
$\displaystyle
\infer[\Rightarrowintro \mathrm{from} 1]{((p \implies r) \land (q
\implies r)) \implies ((p \lor q) \implies r)}{
  \ulcorner (p \implies r) \land (q \implies r) \urcorner^{[1]}
  &
  \infer[\Rightarrowintro \mathrm{from} 2]{(p \lor q) \implies r}{
    \ulcorner p \lor q \urcorner^{[2]}
    &
    \infer[\lor\textrm{-elim}^{[2]}]{r}{
      \ulcorner p \lor q \urcorner^{[2]}
      &
      \raiseproof{12ex}{\infer[\Rightarrowelim]{r}{
      ...
      &
      \hskip 6em \raiseproof{30ex}{\infer[\Rightarrowelim]{r}{
          \ulcorner q \urcorner^{[2]}
          &
...
taking $(p \implies r) \land (q \implies r)$ as assumption [1] and
applying the implication introduction rule to arrive at our
...
bottom up process.  As I have already made assumptions $((p \implies
r) \land (q \implies r))$ [1] and $(p \lor q)$ [2] and they are
related, I start by working with both top-down. I begin with $(p \lor
q)$ [2] and create cases for p and q working towards or elimination.
The p and q sub-branches are marked with [2] as the assumption as
these are just the cases for $(p \lor q)$ [2] \citep[slide
41]{simpson25c}. First, I apply and elimination in each case to get
$(p \implies r)$, $(q \implies r)$ with p, q respectively in the
sub-branches.  From there, I apply implication elimination to arrive
at r in both cases.  This enables me to use the or elimation together
with $(p \lor q)$ [2] to simplify to r.  At this stage, I do not
discharge assumption [2], as this is deferred until the next step. I
use implication introduction together with $(p \lor q)$ [2] and r
from the or elimination to discharge assumption [2], which connects
the top-down and bottom-up process. All assumptions are properly discharged.

\bigskip

\bigskip

In the other direction, we are seeking to prove $(p \lor q \implies
r)$ $\Rightarrow$ $((p \implies r) \land (q \implies r))$:

\bigskip

\noindent
$\displaystyle
\infer[\Rightarrowintro \mathrm{from} 1]{((p \lor q) \implies r)
\implies ((p \implies r) \land (q \implies r))}{
  ...
  \infer[\land \mathrm{intro}]{((p \implies r) \land (q \implies r))}{
    \infer[\Rightarrowintro \mathrm{from} 2]{p \implies r}{
      \ulcorner p \urcorner^{[2]}
      &
      \infer[\Rightarrowelim]{r}{
        \ulcorner (p \lor q) \implies r \urcorner^{[1]}
        &
    ...
    &
    \infer[\Rightarrowintro \mathrm{from} 3]{q \implies r}{
      \ulcorner q \urcorner^{[3]}
      &
      \infer[\Rightarrowelim]{r}{
        \ulcorner (p \lor q) \implies r \urcorner^{[1]}
...
\implies r)$ as assumption [1] and applying the implication
introduction rule to discharge assumption [1].  This leaves us with
$(p \implies r)$ and $(q \implies r)$, which is derived from $p
\implies r$, $q \implies r$ and the application of the and
introduction rule. Fairly clearly, we need two sub-branches to be
developed top-down. Since we have $(p \lor q \implies r)$ and we want
$p \implies r$ and $q \implies r$, I start with p [2] and q [3] to
start the top-down sub-trees.  I struggled here until I recalled that
I could use the or intro rule to satisfy $(p \lor q)$.  I also had to
recall that this does not generate additional assumptions to
discharge \citep[slide 38]{simpson25c}. From here, it was not
difficult to use implication elimation to arrive at r in both
...
This gave me $p \implies r$, $q \implies r$ and I used the and
...
\noindent
$\displaystyle
\infer[\Leftrightarrowintro]{((p \implies r) \land (q \implies r))
...
\in R$ $\circ$ S $\Leftrightarrow$ $\exists y : Y @ x \mapsto y \in R
\land y \mapsto z \in S$ \citep[page 107]{simpson-notes}.
...
\[
  \begin{array}{ll@{\hspace{2em}}l}
    ...
    R \land x \mapsto y \in S) \land y \mapsto z \in T &
    [\mbox{definition of $\circ$}] \\
    &\Leftrightarrow \exists y : Y @ \exists x : X @ w \mapsto x \in
    R \land x \mapsto y \in S \land y \mapsto z \in T & [\mbox{x is
    $\lnot$ free $\in$ y $\mapsto$z $\in$ T}] \\
    &\Leftrightarrow \exists x : X @ w \mapsto x \in R \land (\exists
    y : Y @ x \mapsto y \in S \land y \mapsto z \in T) & [\mbox{y is
    $\lnot$ free $\in$ w $\mapsto$x $\in$ R}] \\
...
\noindent
\hangindent=2em
(a) $shows \subseteq \dom show\_episodes \land \dom show\_episodes
\subseteq shows$

\medskip
\noindent
\hangindent=2em
(b) $\forall es : \ran show\_episodes @ \exists n : \nat @ es = 1 \upto n$

\medskip
\noindent
\hangindent=2em
(c) $\forall s : \dom show\_episodes @ \forall e : show\_episodes(s) @ \\
\quad (s, e) \in \dom episode\_info \iff s \in \dom show\_episodes$

\medskip
\noindent
\hangindent=2em
(d) $shows \subseteq \dom show\_info \land \dom show\_info \subseteq shows$
...
\noindent
\hangindent=2em
(f) $\forall s : ShowId @ \forall t_1, t_2 : Timestamp @ \forall e_1,
e_2 : EpisodeId @ \\
\quad ((s, e_1) \in \dom episode\_info \land (s, e_2) \in \dom
episode\_info) \land \\
\quad ((e_1 < e_2) \implies (snd(episode\_info(s, e_1)) \leq
...
\noindent
$\forall p : PossiblePlaylist @ length(played(p)) +
...
Playlist is of type seq((ShowId cross EpisodeId) cross N).

\bigskip

\bigskip

The base case occurs when p = $\langle \rangle$. Therefore, we must first prove
...
\[
  \begin{array}{ll@{\hspace{2em}}l}
    ...
    \rangle) & [\mbox{played($\langle \rangle$)=$\langle
    \rangle$$\land$ unplayed($\langle \rangle$)=$\langle \rangle$}] \\
...
prove the inductive hypothesis, which is an implication.
...
\quad length(played(ple \cat t)) + length(unplayed(ple \cat t)) =
length(ple \cat t)$

\[
  \begin{array}{ll@{\hspace{2em}}l}
    & length(played(ple \cat t)) + length(unplayed(ple \cat t)) \\
    &\Leftrightarrow snd(ple) + length(t) \\
    &\Leftrightarrow length(ple \cat t) & [\mbox{length($\langle ple
    \rangle$⌢ pl)=snd(ple)+ length(pl)}]
  \end{array}
\]

\bigskip

A playlist element is of type ((dom status) cross N) and status is of
type (PlayedOrNot, SavedOrNot). The inductive hypothesis proof begins
by establishing that there are two mutually exclusive and exhaustive
cases: one where the playlist element's PlayedOrNot free type value
is Played and another where it is NotP.  The proof will establish
that for both cases, the same conclusion can be proven. In addition,
the inductive proof relies upon the inductive hypothesis
