...
We are defining a function that will tell us the difference in length between the longest and shorest sequences from an input type that is a non-empty sequences of sequences. The input could be $<$ $\langle x, y, z \rangle$, $\langle \rangle$ $>$ or $<$ $\langle x \rangle$ $>$ for example. I specify three generic functions: maxSeq returns the longest sequence from a non-empty sequence of sequences; minSeq returns the shortest sequence from a non-empty sequence of sequences; and maxDiff, which fulfills the specification for Question 4.
