...
Using this abbreviation, we can simplify our free type definition.
...
An example for such a List would include (List((1, 2), (3600, NotP,
Saved)), List((1, 1), (3600, Played, Saved)), List(nil)).
...
Having thus simplified our notation, we define the induction
principle for List, which builds on the example for a List of natural
numbers found in section 13.3 of the course notes \citep[page
162]{simpson-notes}.
...
\noindent
$\displaystyle
\infer{\forall l : List @ \power l}{
  \power nil
  &
  \forall e : ListElement @ \forall l : List @ \power l \implies
  \power (join(e, l))
}
$
...
Fuzz does not support validation of Z with tuple projection (e.g.,
x.1). Therefore, we define generic access functions for each element
of any triple as we need access to various elements of (N cross
PlayedOrNot cross SavedOrNot) for our recursive functions.
