...
The Base Case occurs when p = $\langle \rangle$. Therefore, we must
first prove length(played($\langle \rangle$)) +
length(unplayed($\langle \rangle$)) = length($\langle \rangle$).
...
\[
  \begin{array}{ll@{\hspace{2em}}l}
    ...
    \rangle) & [\mbox{played ( < > ) = < > $\land$ unplayed ( < > ) = < >}] \\
    &\Leftrightarrow 0 + 0 & [\mbox{length ( < > ) = 0}] \\
    &\Leftrightarrow length(\langle \rangle) & [\mbox{length ( < > ) = 0}]
  \end{array}
\]
