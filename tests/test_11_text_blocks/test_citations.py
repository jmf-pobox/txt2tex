...
class TestCitationProcessing:
    ...
    def test_basic_citation(self) -> None:
        ...
        text = "TEXT: See [cite spivey92] for details."
        lexer = Lexer(text)
        ...
        assert "\\citep{spivey92}" in latex
        assert "[cite spivey92]" not in latex

    def test_citation_with_slide(self) -> None:
        """Test citation with slide locator."""
        text = "TEXT: See [cite spivey92 slide 20]."
        lexer = Lexer(text)
        tokens = lexer.tokenize()
        parser = Parser(tokens)
        ast = parser.parse()
        generator = LaTeXGenerator()
        latex = generator.generate_document(ast)
        assert "\\citep[slide 20]{spivey92}" in latex
        assert "[cite spivey92 slide 20]" not in latex
    ...
    def test_multiple_citations_in_paragraph(self) -> None:
        ...
        text = "TEXT: See [cite spivey92] land [cite spivey92 p. 42]."
        lexer = Lexer(text)
        tokens = lexer.tokenize()
        parser = Parser(tokens)
        ast = parser.parse()
    ...
    def test_citation_not_in_text_block(self) -> None:
        ...
        text = "PURETEXT: [cite spivey92] should remain literal."
        lexer = Lexer(text)
        tokens = lexer.tokenize()
        parser = Parser(tokens)
        ast = parser.parse()
    ...
    def test_citation_not_in_latex_block(self) -> None:
        ...
        text = "LATEX: [cite spivey92] should remain literal."
        lexer = Lexer(text)
        tokens = lexer.tokenize()
        parser = Parser(tokens)
        ast = parser.parse()
    ...
    def test_citation_with_complex_locator(self) -> None:
        ...
        text = "TEXT: See [cite spivey92 chapter 3, section 2, slide 45-50]."
        lexer = Lexer(text)
        tokens = lexer.tokenize()
        parser = Parser(tokens)
        ast = parser.parse()
        generator = LaTeXGenerator()
        latex = generator.generate_document(ast)
        assert "\\citep[chapter 3, section 2, slide 45-50]{spivey92}" in latex
    ...
    def test_citation_in_middle_of_sentence(self) -> None:
        ...
        text = "TEXT: The proof technique [cite spivey92] is discussed here."
        lexer = Lexer(text)
        tokens = lexer.tokenize()
        parser = Parser(tokens)
        ast = parser.parse()
    ...
    def test_multiple_citations_with_various_locators(self) -> None:
        ...
        text = (
            "TEXT: See [cite spivey92], [cite spivey92 p. 42], "
