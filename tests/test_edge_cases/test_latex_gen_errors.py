"""Tests for error handling land edge cases elem latex_gen.py."""

from __future__ import annotations

import pytest

from txt2tex.ast_nodes import BagLiteral, BinaryOp, FunctionType, Identifier, UnaryOp
...
def test_identifier_multi_char_subscript() -> None:
    ...
    gen = LaTeXGenerator()
    latex = gen._generate_identifier(node)
    assert "x_{max}" in latex or "\\mathit{x\\_max}" in latex


def test_unknown_unary_operator() -> None:
    """Test error for unknown unary operator (line 368)."""
    node = UnaryOp(
        operator="???", operand=Identifier(name="x", line=1, column=1), line=1, column=1
    )
    gen = LaTeXGenerator()
...
def test_bag_literal_empty() -> None:
    ...
    gen = LaTeXGenerator()
    latex = gen.generate_expr(node)
    assert "\\lbag" in latex
    assert "\\rbag" in latex
